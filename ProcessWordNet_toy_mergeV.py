...
test_snli = load_nli_data("./data/snli_1.0/snli_1.0_train.jsonl", snli=True)
...
Dic_external_know = {}
f_dev = open("snli_dev_relationship_feat.pkl",'rb')
dic_file = pickle.load(f_dev)
        ...
        for word2 in sent_2.split():
            if (word1, word2) in dic_file:
                ...
                if all(ele == 0 for ele in R_lst):
                    ...
                    l_R = 1 # lr_ij
                dic_file[(word1,word2)] = l_R # key: (word1, word2); value: L(R_ij) [should be 1 or 0]
    
    if data_i%5000:
        output = open('snli_train_relationship_feat.pkl', 'wb')
        pickle.dump(dic_file, output)
        output.close()
